...
W = tf.Variable(tf.random_uniform([statespace_size,len(Pong.DIRECTIONS)],0,0.01))
Qout = tf.matmul(inputs1,W)
predict = tf.argmax(Qout,1)

#Below we obtain the loss by taking the sum of squares difference between the target and prediction Q values.
nextQ = tf.placeholder(shape=[1,len(Pong.DIRECTIONS)],dtype=tf.float32)
    ...
    for i in range(num_episodes):
        ...
        while j < 99:
            ...
            if np.random.rand(1) < e:
                a[0] = random.choice(Pong.DIRECTIONS)
            #Get new state and reward from environment
            s1,r,d = Pong.simple_step(a[0])
            #Obtain the Q' values by feeding the new state through our network
            Q1 = sess.run(Qout,feed_dict={inputs1:np.identity(statespace_size)[s1:s1+1]})
            #Obtain maxQ' and set our target value for chosen action.
            maxQ1 = np.max(Q1)
            targetQ = allQ
            targetQ[0,a[0]] = r + y*maxQ1
            #Train our network using target and predicted Q values
            _,W1 = sess.run([updateModel,W],feed_dict={inputs1:np.identity(statespace_size)[s:s+1],nextQ:targetQ})
            rAll += r
            s = s1
            if d == True:
